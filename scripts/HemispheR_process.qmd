...
## cropping the images
cropping all the images because they have a black border on top and bottom.

```{r crop images, eval = FALSE}
# Define the root directory of your images
input_dir <- "/Users/simon/Documents/Master/Masterarbeit/LAMeasurement/Hemispheric_Photos/LT_Ecosense_Contrast/" # Replace with the folder containing subfolders and images
output_dir <- "/Users/simon/Documents/Master/Masterarbeit/LAMeasurement/Hemispheric_Photos/LT_Ecosense_Contrast_processed" # Replace with the output folder path

# Create the output directory if it doesn't exist
if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)

# Get all image file paths recursively (case-insensitive for extensions)
image_files <- list.files(input_dir, pattern = "\\.(jpg|jpeg|png|tif|bmp|JPG|JPEG|PNG|TIF|BMP)$", full.names = TRUE, recursive = TRUE)
...
for (image_path in image_files) {
  ...
  # Crop the image (adjusting from y = 220 to y = height - 280)
  cropped_image <- crop.borders(image, ny = c(220, 280))
  
  # Determine the corresponding output path
  relative_path <- sub(input_dir, "", image_path) # Get the relative path within input_dir
  output_path <- file.path(output_dir, dirname(relative_path)) # Append it to the output_dir
  
  # Create subfolders in the output directory if they don't exist
  if (!dir.exists(output_path)) dir.create(output_path, recursive = TRUE)
  
  # Ensure output file has a lowercase extension
  output_file <- file.path(output_path, sub("\\.JPG$", ".jpg", basename(image_path)))
  
  # Save the cropped image (convert to jpg if needed)
  tryCatch({
...
When using the process_directory function it is important to check if the pattern for the filetype of the image is set right (e.g. '.JPG').
...
process_directory('/Users/simon/Documents/Master/Masterarbeit/LAMeasurement/Hemispheric_Photos/LT_Ecosense_Contrast_processed/WoConifer/')

# process_image('/Users/simon/Documents/Master/Masterarbeit/LAMeasurement/Hemispheric_Photos/LT_Ecosense_processed/LT11/LT11_20240920.JPG', 60)
```
